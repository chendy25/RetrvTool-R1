    ...
    async def get_final_mm_inputs(self, rollings: DataProto):
        ...
        async with self.tokenizer_lock:
            for i in range(rollings.batch['input_ids'].shape[0]):
                ...
                #  # 关键修改：每张图像单独处理，避免批处理导致的 resize 差异
                # if images:
                #     # 分别处理每张图像
                #     all_image_inputs = []
                #     for img in images:
                #         single_img_input = self.processor.image_processor([img], return_tensors="pt")
                #         all_image_inputs.append(single_img_input)

                #     # 合并所有图像的特征
                #     model_inputs = self.processor(
                #         text=[raw_prompt], 
                #         images=images,  # 但仍然一起传入以获得正确的 grid_thw
                #         videos=videos, 
                #         return_tensors="pt"
                #     )
                # else:
