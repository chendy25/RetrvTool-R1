...
class PixelReasonerTool(BaseTool):
    ...
    valid_mcp_func_names = ['zoom_in', 'crop_image_normalized', 'select_frames', 'crop_image',"select_images"]
    ...
    async def conduct_select_images_action_async(self, parameters, env):
        """
        Execute the select images action asynchronously.
        执行select_images动作 - 从候选图像中选择特定的图像进行进一步分析
        """
        valid = False
        missing_parameters = []
        if 'target_images' not in parameters:
            missing_parameters.append('target_images')

        if missing_parameters:
            observation = f"Missing parameters: {', '.join(missing_parameters)}"
        elif not isinstance(parameters['target_images'], list):
            observation = "Invalid target_images format. It should be a list of integers."
        elif 'candidate_images' not in env or env['candidate_images'] is None:
            observation = "No candidate images available. Please ensure candidate_images are provided in extra_field."
        elif not all(isinstance(image, int) and 1 <= image <= len(env['candidate_images']) for image in parameters['target_images']):
            observation = f"Invalid target_images indices. Each index should be an integer between 1 and the number of candidate images ({len(env['candidate_images'])})."
        else:
            try:
                # 从候选图像中选择指定的图像
                target_image_sources = [env['candidate_images'][image - 1] for image in parameters['target_images']]

                # Process all images concurrently
                # 并发处理所有选中的图像
                selected_images = await self._process_multiple_images(target_image_sources)

                # 构建观察结果
                image_width, image_height = selected_images[0].size
                num_images = len(selected_images)
                observation = {
                    'obs': f"Here are the selected candidate images for detailed analysis. (Image Size: {image_width}x{image_height}, Numbered 1 to {num_images}):" + "<image>" * len(selected_images),
                    'image': [encode_image_url(img) for img in selected_images]
                }
                valid = True
            except Exception as e:
                observation = f"Error processing images: {str(e)}"
                print(f"Error processing select images action: {str(e)}; parameters: {parameters}")
    ...
    async def _conduct_action_async(self, trajectory_id: str, action: str, extra_field: Dict[str, Any]):
        ...
        # 初始化候选图像列表
        if env['candidate_images'] is None and "candidate_images" in extra_field:
            env['candidate_images'] = [Path(x) for x in extra_field["candidate_images"]]
        ...
        if not is_valid:
            ...
            #新增select_images动作
            elif parsed_action['name'] == 'select_images':
                try:
                    observation, valid = await self.conduct_select_images_action_async(parsed_action['arguments'], env)
                except Exception as e:
                    observation = f"Error processing select images action: {str(e)}"
                    valid = False
                    print(f"Error processing select images action: {str(e)}; parameters: {parsed_action['arguments']}")
            else:
    ...
    #新增select_images同步动作
    def conduct_select_images_action(self, parameters, env):
        """
        Synchronous wrapper for select images action.
        """
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
            return loop.run_until_complete(self.conduct_select_images_action_async(parameters, env))
        finally:
            loop.close()
