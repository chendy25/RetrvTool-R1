...
dataset_name=pixel_reasoner/LamRA_RL_Data/
train_data=[$(pwd)/data/${dataset_name}/lamra_train_300K.parquet]
val_data=[$(pwd)/data/${dataset_name}/lamra_train_300K.parquet]
...
max_obs_length=16384 # should be big to avoid any truncation of image tokens which will cause error
...
gpu_memory_utilization=0.98 # higher gpu_memory_utilization will likely cause the vllm to OOM and get stuck, so set it to a lower value like 0.4 or 0.5
